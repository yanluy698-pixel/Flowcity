...
主项目内置受保护的轻量后台入口，用于数据治理、Mock 供给检查和学习提案审核。它不是普通用户入口，而是运营、开发者和评审查看系统能力的后台页面：
...
- 展示 open-access 二级商圈不需要库存影子表，路线和团购券属于扩展动态对象。
- 展示确认前 Mock 风险来源，包括无票、无座、排队变长、路线变慢和套餐库存变化。
