...
- 正式前端：[https://flowcity.vercel.app](https://flowcity.vercel.app)
- 管理台入口：[https://flowcity.vercel.app/#admin](https://flowcity.vercel.app/#admin)
- 后端健康检查：[https://flowcity-api.onrender.com/health](https://flowcity-api.onrender.com/health)
- 代码仓库：[https://github.com/yanluy698-pixel/Flowcity](https://github.com/yanluy698-pixel/Flowcity)

## 管理台说明

管理台不是普通用户页面，而是给运营、开发者和评审查看系统治理能力的后台入口。它用于展示：

- POI / Mock 数据：活动、餐厅、二级商圈、路线、余票座位、排队、团购和运行时状态。
- 供给健康：每个商圈的活动、餐饮、补位点、价格层和缺口提示。
- 数据治理：POI 来源、置信度、事实标签、约束标签和 open-access 二级商圈状态。
- 履约模拟：确认前动态状态、无票/无座/路线变慢/团购库存变化等 Mock 风险。
- 自进化审核：开放假设聚类、候选样本、确认率、删除率、语义凝聚度和审核状态。

评审只读 Token：

```text
flowcity-reviewer-readonly
```

只读 Token 只能查看数据，不能新增、保存、删除 POI，也不能批准或拒绝自进化候选。写权限由部署环境变量 `FLOWCITY_ADMIN_TOKEN` 单独配置，不写入公开文档。
